...
public class BankImpl implements Bank {
    ...
    private AcquiredAccount acquire(int index, Op op) {
        ...
        while (true) {
            ...
            if (!account.invokeOperation()) {
                AcquiredAccount acquiredAccount = new AcquiredAccount(account.amount, op);
                if (accounts.compareAndSet(index, account, acquiredAccount) && !op.completed) {
                    return acquiredAccount;
                }
            }
        }
    ...
    private class TransferOp extends Op {
        ...
        void invokeOperation() {
            ...
            acquire(Math.min(fromIndex, toIndex), this);
            acquire(Math.max(fromIndex, toIndex), this);
            AcquiredAccount from = acquire(fromIndex, this);
            AcquiredAccount to = acquire(toIndex, this);
            ...
            completed = true;
            release(Math.max(fromIndex, toIndex), this);
            release(Math.min(fromIndex, toIndex), this);
        }
    }
}
